...
def fitness_function_2_student(candidate_board, initial_board, env):
    """
    Fitness basado en el conteo de dígitos únicos en cada fila, columna y subcuadrado.
    Máximo en 9×9: 9 filas + 9 columnas + 9 bloques, cada uno con 9 dígitos únicos → 243.
    """
    # Penalizar cambios en pistas iniciales
    if check_initial_clues(candidate_board, initial_board) > 0:
        return -1e6

    fitness = 0
    n = int(SUDOKU_SIZE**0.5)

    # Filas
    for i in range(SUDOKU_SIZE):
        fitness += len(set(candidate_board[i]))

    # Columnas
    for j in range(SUDOKU_SIZE):
        col = [candidate_board[i][j] for i in range(SUDOKU_SIZE)]
        fitness += len(set(col))
    ...
    for bi in range(n):
        for bj in range(n):
            ...
            for di in range(n):
                for dj in range(n):
                    block.append(candidate_board[bi*n + di][bj*n + dj])
...
def fitness_function_3_student(candidate_board, initial_board, env):
    """
    Fitness basado en penalizar errores: 
    F = (máximo único total) - (errores en filas + columnas + subcuadrados).
    Máximo único total = 9 filas + 9 columnas + 9 bloques * 9 celdas = 243.
    """
    # Penalizar cambios en pistas iniciales
    if check_initial_clues(candidate_board, initial_board) > 0:
        return -1e6

    # Calcular errores con tus funciones auxiliares
    row_err      = calculate_row_errors(candidate_board)
    col_err      = calculate_col_errors(candidate_board)
    block_err    = calculate_subgrid_errors(candidate_board)
    total_errors = row_err + col_err + block_err

    # Definimos el fitness restando los errores al total posible
    max_unique = SUDOKU_SIZE * 3 * SUDOKU_SIZE  # 9*3*9 = 243
    fitness = max_unique - total_errors
